    ...
    for (UIButton *button in self.buttons) {
        ...
        if (self.isColor) { //彩色
            ...
            if (button.tag!=107) {
                ...
                self.colorView.bgImageView.image = [UIImage imageNamed:@"icon_ctrl_w_mode"];
            }
            if (button.tag>=104) {
                button.hidden = YES;
...
- (NSArray *)warmColdNameArray{
    if (!_warmColdNameArray) {
        _warmColdNameArray = @[LCSTR("3000k"), LCSTR("3500k"), LCSTR("4000k"), LCSTR("4500k"), LCSTR("4800k"), LCSTR("5000k"), LCSTR("5500k"), LCSTR("Color")];
    }
...
- (NSArray *)dataSource{
    if (!_dataSource) {
        _dataSource = [WSTColorModel selectFromClassPredicateWithFormat:@"where modelName = 'RGBW'"];
        if (_dataSource.count == 0) {
            [WSTColorModel insertDefaultModelWithRGBW];
            _dataSource = [WSTColorModel selectFromClassPredicateWithFormat:@"where modelName = 'RGBW'"];
        }
    }
    return _dataSource;
