...
- (void)masLayoutSubviews{
    ...
    for (int j = 0;j<1;j++) {
        for (int i = 0; i<4; i++) {
            ...
            button.backgroundColor = [UIColor colorWithHue:WCConfig_All[button.tag-100+4].h saturation:WCConfig_All[button.tag-100+4].s brightness:WCConfig_All[button.tag-100+4].b alpha:1];
...
- (ColorPickerView *)colorView{
    if (!_colorView) {
        ...
        _colorView.bgImageView.image = [UIImage imageNamed:@"color_c"];
...
- (NSArray *)dataSource{
    if (!_dataSource) {
        _dataSource = [WSTColorModel selectFromClassPredicateWithFormat:@"where modelName = 'C'"];
        if (_dataSource.count == 0) {
            [WSTColorModel insertDefaultModelWithC];
            _dataSource = [WSTColorModel selectFromClassPredicateWithFormat:@"where modelName = 'C'"];
        }
    }
    return _dataSource;
