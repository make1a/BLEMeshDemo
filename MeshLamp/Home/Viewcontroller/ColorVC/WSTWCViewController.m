...
@interface WSTWCViewController ()<ColorPickerDelegate,WZBlueToothDataSource>
@property (nonatomic,strong) WSTWCView  *containerView;

@end

@implementation WSTWCViewController
...
- (WSTWCView *)containerView{
    if (!_containerView) {
        _containerView = [[WSTWCView alloc]init];
        ...
        _containerView.pressButton = ^(CGFloat h, CGFloat s, CGFloat b, BOOL isColor) {
            ...
            [[WZBlueToothDataManager shareInstance]setHSBWithAddress:(int32_t)weakSelf.devAddr isGroup:weakSelf.isRoom WithHue:0 Saturation:0 Brightness:0 Warm:h Cold:s Lum:1 Delay:0];
