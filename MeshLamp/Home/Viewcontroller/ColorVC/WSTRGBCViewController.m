...
@interface WSTRGBCViewController ()<ColorPickerDelegate,WZBlueToothDataSource>
@property (nonatomic,strong) WSTRGBCView  *containerView;

@end

@implementation WSTRGBCViewController
...
//        [[WZBlueToothDataManager shareInstance]setColdWithAddress:(int32_t)self.devAddr isGroup:self.isRoom WithCold:colorPickerView.saturationValue <= 0.05 ? 0 : colorPickerView.saturationValue Delay:0];
    }
}
...
- (WSTRGBCView *)containerView{
    if (!_containerView) {
        _containerView = [[WSTRGBCView alloc]init];
        ...
        _containerView.pressButton = ^(CGFloat h, CGFloat s, CGFloat b, BOOL isColor) {
            ...
            if (isColor) {
                ...
                [[WZBlueToothDataManager shareInstance]setColdWithAddress:(int32_t)weakSelf.devAddr isGroup:weakSelf.isRoom WithCold:b Delay:0];
            }
        };
